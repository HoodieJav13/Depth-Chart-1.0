...
assignment data attached to the eight retired defensive position ids — see
"Defensive formation: 4-2-5" for why that requires no migration code and what it costs.
...
The same mechanism that makes migration unnecessary makes retired defensive ids **irreversible**
the moment the shared document is normalized and later saved. This spec was written when the
defensive chart was empty; a coach may have entered assignments since.
...
2. If any defensive position holds a player: **stop and report it.** Do not proceed on the
   assumption that the chart is still empty, and **do not rely on an in-app snapshot to preserve
   retired defensive position ids**.
3. If empty: proceed. A named snapshot may still be created through the existing
   `SnapshotManager` as a general offense/roster restore point, but it is **not** a rollback
   mechanism for retired defensive ids because snapshot state is normalized against the current
   `formationConfig` when loaded.
4. If a true pre-change defensive backup is ever required, preserve the raw Firestore depth-chart
   document outside the app before changing the formation config.
