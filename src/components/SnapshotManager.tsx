...
export const SnapshotManager = ({ store, onClose }: SnapshotManagerProps) => {
  ...
  const refresh = async () => {
    setSnapshots(await store.listSnapshots());
  };
  ...
  const create = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        caught instanceof Error ? caught.message : "Snapshot could not be saved.",
      );
    } finally {
      setBusy(false);
    }
  ...
  const restore = async (snapshot: DepthChartSnapshot) => {
    if (
      !window.confirm(
        `Restore “${snapshot.name}”? This replaces the current shared chart.`,
      )
    ) {
      return;
    }
  ...
  return (
    <div
      className="dialog-backdrop workflow-overlay"
      role="presentation"
      onMouseDown={onClose}
    >
      <section
        className="workflow-dialog snapshot-dialog"
        role="dialog"
        aria-modal="true"
